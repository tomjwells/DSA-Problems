...
def dfsHelper(s1: str, s2: str, i1: int, i2: int) -> int:
...
def memoization(s1: str, s2: str):
  N, M = len(s1), len(s2)
  cache = [[-1] * M for _ in range(N)]
  return memoHelper(s1, s2, 0, 0, cache)


def memoHelper(s1: str, s2: str, i1: int, i2: int, cache: List[List[int]]):
  if i1 == len(s1) or i2 == len(s2):
    return 0
  if cache[i1][i2] != -1:
    return cache[i1][i2]
