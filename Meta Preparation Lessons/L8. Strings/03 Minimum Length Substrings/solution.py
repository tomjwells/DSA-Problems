...
def is_substring(candidate_dict: dict[str, int], t_dict: dict[str, int]):
  for key, _value in t_dict.items():
...
def add_to_dict(char: str, t_dict: dict[str, int]):
  if char in t_dict:
    t_dict[char] += 1
  else:
    t_dict[char] = 1
...
def min_length_substring(s: str, t: str) -> int:
